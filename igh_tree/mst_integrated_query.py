...
def main(argv):
    ...
    wdir = os.path.dirname(outfile_query_basename)
    ...
    outfile_query = wdir + "/mst.query.temp." + str(i_worker)
    outfile_query_field_dict = wdir + "/mst.query.field_dict.json.temp." + str(i_worker)
...
def query(patient_uid, clustering_uid, num_workers, i_worker, outfile,
    ...
    query = """
            SELECT s.uid, s.lineage_uid, s.sequence_string_uid,
                   ...
                   s.V_germline, s.D_germline, s.J_germline,
    ...
    field_names = ["sequence_uid", "lineage_uid", "sequence_string_uid", "V_seq", "D_seq", "J_seq",
                   "V_germline", "D_germline", "J_germline", "subisotype", "abundance"]
